<?php
/**
 * PackageUpdateBlacklistAddAction
 *
 * Add a blacklist item (via AJAX)
 *
 * PHP Version 5
 *
 * LICENSE:
...
class PackageUpdateBlacklistAddAction extends AbstractAction {
	public $packageID = 0;
	...
	public function execute() {
		...
		$wildcardIndex = strpos($this->blacklistPackageVersion, '*');
		$wildcardOkay = (($wildcardIndex === false) || ($wildcardIndex === (strlen($this->blacklistPackageVersion) - 1)));
		if(($this->blacklistPackageVersion === '*') || !$wildcardOkay)
			throw new IllegalLinkException();

		if(PackageUpdateBlacklistEditor::add($this->packageID, $this->blacklistPackageVersion)) {
